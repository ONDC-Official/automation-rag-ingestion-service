...
There are **27** validation rules organized into **7** top-level group(s).
...
This is a group of **2** sub-validation(s) that all must pass: REQUIRED_ORDER_PROVIDER_ID and REQUIRED_ORDER_ITEMS.

---

**REQUIRED_ORDER_PROVIDER_ID**
`group: ON_STATUS_ORDER | type: leaf`
...
This is a group of **1** sub-validation(s) that all must pass: REQUIRED_FULFILLMENT_TYPE.

---

**REQUIRED_FULFILLMENT_TYPE**
`group: ON_STATUS_FULFILLMENTS | type: leaf`
...
This is a group of **1** sub-validation(s) that all must pass: VALID_FULFILLMENT_STATE_CODE.

> **Skip if:**
> - $.message.order.fulfillments[*].state.descriptor.code must **not** be present in the payload

---

**VALID_FULFILLMENT_STATE_CODE**
`group: ON_STATUS_FULFILLMENT_STATE | type: leaf`

- At least one of $.message.order.fulfillments[*].state.descriptor.code must be in ["INITIATED", "SANCTIONED", "DISBURSED", "PENDING", "REJECTED", "COMPLETED", "CONSENT_REQUIRED", "APPROVED", "DISPATCHED"]

---

## ON_STATUS_XINPUT_FORM_RESPONSE_STATUS_NOT_REJECTED

This is a group of **1** sub-validation(s) that all must pass: If_Validation_Result_is_REJECTED_flow_cant_move_forward.
...
**If_Validation_Result_is_REJECTED_flow_cant_move_forward**
`group: ON_STATUS_XINPUT_FORM_RESPONSE_STATUS_NOT_REJECTED | type: leaf`

- All elements of $.message.order.items[*].xinput.form_response.status must be in ["COMPLETED", "OFFLINE_PENDING", "APPROVED", "SUCCESS"]

---

## ON_STATUS_PAYMENTS

This is a group of **2** sub-validation(s) that all must pass: REQUIRED_PAYMENT_TYPE and REQUIRED_PAYMENT_STATUS.
...
`group: ON_STATUS_PAYMENTS | type: leaf`
...
> **Skip if:**
>
>     - $.message.order.quote.breakup[*].title is not in the payload
