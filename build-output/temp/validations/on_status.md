...
There are **77** validation rules organized into **9** top-level group(s).
...
`group: ON_STATUS_FULFILLMENTS > REQUIRED_FULFILLMENT_ITEMS | type: leaf`
...
`group: ON_STATUS_FULFILLMENTS > REQUIRED_FULFILLMENT_ITEMS > REQUIRED_CREDS | type: leaf`
...
`group: ON_STATUS_FULFILLMENTS > REQUIRED_FULFILLMENT_ITEMS > REQUIRED_CUSTOMER_CONTACT_DETAILS | type: leaf`
...
`group: ON_STATUS_FULFILLMENTS > REQUIRED_FULFILLMENT_ITEMS > REQUIRED_CUSTOMER_ORG_DETAILS | type: leaf`
...
`group: ON_STATUS_FULFILLMENTS > VALID_FULFILLMENT_ITEMS | type: leaf`
...
`group: ON_STATUS_FULFILLMENTS > VALID_FULFILLMENT_TAGS | type: leaf`
...
`group: ON_STATUS_FULFILLMENTS > VALID_FULFILLMENT_TAGS | type: leaf | scope: $.message.order.fulfillments[*].tags[?(@.descriptor.code == 'CHECKLISTS')]`
