...
There are **60** validation rules organized into **10** top-level group(s).
...
## ON_SEARCH_CATEGORIES_PL

This is a group of **5** sub-validation(s) that all must pass: REQUIRED_CATEGORIES_NAME, REQUIRED_CATEGORIES_CODE, VALID_ENUM_CATEGORIES_CODE, REQUIRED_CATEGORIES_ID, and REQUIRED_CATEGORIES_PARENT_ID. It validates the `$.message.catalog.providers[*].categories[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.

---

**REQUIRED_CATEGORIES_NAME**
`group: ON_SEARCH_CATEGORIES_PL | type: leaf`
...
- At least one of $.message.catalog.providers[*].categories[*].descriptor.code must be in ["PERSONAL_LOAN", "BUREAU_LOAN", "AA_PERSONAL_LOAN", "GOLD_LOAN", "AA_LOAN", "LOAN", "PERSONAL_INFORMATION"]
...
This is a group of **7** sub-validation(s) that all must pass: REQUIRED_PAYMENT_COLLECTED_BY, VALID_PAYMENT_COLLECTED_BY_ENUM, REQUIRED_BUYER_FINDER_FEES_TAG, REQUIRED_BUYER_FINDER_FEES_LIST, VALID_BUYER_FINDER_FEES_TYPE_VALUE, REQUIRED_SETTLEMENT_TERMS_TAG, and REQUIRED_SETTLEMENT_TERMS_LIST. It validates the `$.message.catalog.providers[*].categories[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.
...
- At least one of $.message.catalog.providers[*].payments[*].tags[*].descriptor.code must be in ["BUYER_FINDER_FEES", "SETTLEMENT_TERMS"]
...
This is a group of **8** sub-validation(s) that all must pass: REQUIRED_ITEM_ID, REQUIRED_ITEM_CATEGORY_IDS, REQUIRED_ITEM_DESCRIPTOR_NAME, REQUIRED_GENERAL_INFO_TAG_GROUP, REQUIRED_GENERAL_INFO_LIST, REQUIRED_XINPUT, REQUIRED_XINPUT_FORM_URL, and VALID_XINPUT_HEADINGS_ENUM. It validates the `$.message.catalog.providers[*].categories[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.

---

**REQUIRED_ITEM_ID**
`group: ON_SEARCH_ITEMS_PL | type: leaf`
...
`group: ON_SEARCH_ITEMS_PL | type: leaf | scope: $.message.catalog.providers[*].items[*].tags[?(@.descriptor.code=='GENERAL_INFO')]`
...
- At least one of $.message.catalog.providers[*].items[*].xinput.head.headings[*] must be in ["KYC", "KYC_OFFLINE", "JOURNEY_OFFLINE", "SET_LOAN_AMOUNT", "PERSONAL_INFORMATION", "ACCOUNT_INFORMATION", "EMANDATE", "LOAN_AGREEMENT"]

---

## ON_SEARCH_CATEGORIES_CC

This is a group of **4** sub-validation(s) that all must pass: REQUIRED_CATEGORIES_NAME, REQUIRED_CATEGORIES_CODE, VALID_ENUM_CATEGORIES_CODE, and REQUIRED_CATEGORIES_ID. It validates the `$.message.catalog.providers[*].categories[?(@.descriptor.code=='CREDIT_CARD')]` path in the payload.

---

**REQUIRED_CATEGORIES_NAME**
`group: ON_SEARCH_CATEGORIES_CC | type: leaf`
...
- All elements of $.message.catalog.providers[*].categories[*].descriptor.code must be in ["CARD", "CREDIT_CARD", "PREMIUIM_CARDS", "TRAVEL_CARDS", "REWARDS_CARDS", "LIFESTYLE_CARDS"]
...
This is a group of **8** sub-validation(s) that all must pass: REQUIRED_ITEM_ID, REQUIRED_ITEM_CATEGORY_IDS, REQUIRED_ITEM_DESCRIPTOR_NAME, REQUIRED_GENERAL_INFO_TAG_GROUP, REQUIRED_GENERAL_INFO_LIST_CC, REQUIRED_XINPUT, REQUIRED_XINPUT_FORM_URL, and VALID_XINPUT_HEADINGS_ENUM. It validates the `$.message.catalog.providers[*].categories[?(@.descriptor.code=='CREDIT_CARD')]` path in the payload.

---

**REQUIRED_ITEM_ID**
`group: ON_SEARCH_ITEMS_CC | type: leaf`
...
**REQUIRED_GENERAL_INFO_LIST_CC**
`group: ON_SEARCH_ITEMS_CC | type: leaf | scope: $.message.catalog.providers[*].items[*].tags[?(@.descriptor.code=='GENERAL_INFO')]`

- All elements of $.message.catalog.providers[*].items[*].tags[?(@.descriptor.code=='GENERAL_INFO')].list[*].descriptor.code must be in ["WELCOME_BENEFIT", "ANNUAL_FEE", "JOINING_FEE", "REWARDS", "TRAVEL_BENEFITS", "LIFESTYLE_BENEFITS", "EVERYDAY_BENEFITS", "TNC_LINK", "APR"]
...
- All elements of $.message.catalog.providers[*].tags[?(@.descriptor.code=='CONTACT_INFO')].list[*].descriptor.code must be in ["GRO_NAME", "GRO_EMAIL", "GRO_CONTACT_NUMBER", "GRO_DESIGNATION", "GRO_ADDRESS", "CUSTOMER_SUPPORT_LINK", "CUSTOMER_SUPPORT_CONTACT_NUMBER", "CUSTOMER_SUPPORT_EMAIL"]
