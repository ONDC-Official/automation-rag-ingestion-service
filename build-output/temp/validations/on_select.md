...
There are **68** validation rules organized into **15** top-level group(s).
...
This is a group of **2** sub-validation(s) that all must pass: REQUIRED_CONSENT_INFO_TAG and REQUIRED_CONSENT_HANDLER. It validates the `$.message.order.items[*].tags[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.
...
This is a group of **4** sub-validation(s) that all must pass: REQUIRED_XINPUT_FORM, REQUIRED_XINPUT_FORM_RESPONSE, VALID_XINPUT_FORM_RESPONSE_STATUS, and REQUIRED_XINPUT_FORM_RESPONSE_SUBMISSION_ID. It validates the `$.message.order.items[*].tags[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.
...
## ON_SELECT_CONSENT_XINPUT_CC

This is a group of **1** sub-validation(s) that all must pass: REQUIRED_XINPUT_FORM. It validates the `$.message.order.items[*].tags[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.

---

**REQUIRED_XINPUT_FORM**
`group: ON_SELECT_CONSENT_XINPUT_CC | type: leaf`
