...
This is a group of **4** sub-validation(s) that all must pass: REQUIRED_ITEM_ID, REQUIRED_ITEM_DESCRIPTOR_CODE, REQUIRED_ITEM_PRICE, and REQUIRED_LOAN_INFO_TAG. It validates the `$.message.order.items[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.
...
This is a group of **3** sub-validation(s) that all must pass: REQUIRED_QUOTE_ID, REQUIRED_QUOTE_PRICE, and REQUIRED_QUOTE_BREAKUP. It validates the `$.message.order.items[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.
...
- At least one of $.message.order.fulfillments[*].state.descriptor.code must be in ["DISBURSED", "SANCTIONED", "DISPATCHED"]
...
This is a group of **3** sub-validation(s) that all must pass: REQUIRED_PAYMENT_TYPE, REQUIRED_PAYMENT_STATUS, and VALID_PAYMENT_STATUS_ENUM. It validates the `$.message.order.items[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.
...
This is a group of **3** sub-validation(s) that all must pass: REQUIRED_DOCUMENT_DESCRIPTOR_CODE, REQUIRED_DOCUMENT_URL, and REQUIRED_DOCUMENT_MIME_TYPE. It validates the `$.message.order.items[?(@.descriptor.code=='PERSONAL_LOAN')]` path in the payload.
...
> **Skip if:**
>
>     - $.message.order.cancellation_terms[*].fulfillment_state.descriptor.code is not in the payload
